...
export default function App() {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const freshChat = await db.chats.get(activeChat.id);
      if (freshChat) {
         await db.chats.put({
          ...freshChat,
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            {activeChat?.messages.length === 0 ? (
              ...
              activeChat?.messages.map((message) => (
                <div 
                  key={message.id} 
                  className={`flex gap-4 fade-in ${message.role === 'user' ? 'flex-row-reverse ml-auto max-w-[80%]' : 'max-w-[80%]'}`}
                >
                  ...
                  <div className={`rounded-2xl p-4 shadow-sm border ${
                    ...
                    {message.role === 'assistant' ? (
                      ...
                      <div className="whitespace-pre-wrap leading-relaxed text-[0.95rem] text-[#2D2926]">
                        {message.content}
                      </div>
                    )}
                  </div>
